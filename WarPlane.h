...
public:
    WarPlane(float x, float y, float ac, float maxVel, int hp, int le, int wi, bool col, std::string &nat, int numAmmo, int ammoDamage) : Vehicle(x, y, ac, maxVel, hp, le, wi,col, nat),currentCoolDown(currCoolDown),coolDown(coolDown), numAmmo(numAmmo),ammoDamage(ammoDamage) {};
